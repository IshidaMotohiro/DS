...
```{python read_titanic_train_data, echo =FALSE, cache=TRUE}
import pandas as pd
import numpy as np
...
![](images/Kaggle_compete.png)

\  


ALL Categoriesから Getting Started を選択

\  


![](images/Kaggle_started.png)

\  

初心者向けの Compete 一覧が表示されるので、ここからTitanicを選択。
...
ただ、このページで、データの変数について、ヒストグラムによる概要が確認できる。
...
```{python titanic_remove_vars, cache=TRUE}
...
```{python titanic_alone, cache=TRUE}
train['Alone']=np.where((train["SibSp"]+train["Parch"])>0, 0, 1)
train.drop(['SibSp', 'Parch'], axis=1, inplace=True)
```


```{python titanic_middle_data, echo=FALSE, eval=FALSE}
import pickle
f = open('data/train','wb')
pickle.dump(train,f)
f.close
```
...
```{python titanic_make_dummies_from_sex}
...
```{python titanic_make_dummies_training, cache =TRUE, eval=FALSE}
training = pd.get_dummies(train, columns=["Pclass","Embarked","Sex"], drop_first=True)
training
```


```{python titanic_make_dummies_from_sex_not_show_, echo=FALSE, eval=TRUE}
import pickle
f = open('data/training','rb')
...
```{python titanic_train_data_StandardScaler, cache =TRUE}
...
```{python titanic_age_fare_Std, cache =TRUE}
...
```{python titanic_fit_logistcs, cache =TRUE}
...
```{python eval_titanic_train_Data, cache =TRUE}
...
```{python read_test_csv, echo=FALSE, cache =TRUE}
import pandas as pd
...
```{python titanic_test_data_is_null}
test.isnull().sum()
```
...
```{python titanic_train_data_StandardScaler2, echo=FALSE, cache =FALSE}
import pickle
...
```{python read_test_csv_np, echo=FALSE, cache =FALSE}
import pandas as pd
import numpy as np
test = pd.read_csv('data/test.csv')
```
...
```{python prepare_titanic_test_Data, cache =FALSE}
...
```{python titanic_eval_test_data, eval=FALSE, cache=TRUE}
cols = ["Age","Fare","Alone","Pclass_2","Pclass_2","Embarked_Q","Embarked_S","Sex_male"] 
X_test=testing[cols]
...
テストデータの応答変数(Survived)は与えられていないので、これを回答として提出する。

予測結果を保存してコミットする準備を行う。
